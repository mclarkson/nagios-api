...
my $DepCount = keys %{$json_response->{"deployment"}};

## test values against threshhold, and print status and exit
$n_status="OK";
$n_output =  "Currently Deployed: $DepCount apps";


  if ( ($o_crit != 0 ) && ($o_crit > $DepCount ) )  {
    $n_status="CRITICAL";
    $n_output .= " :  " . $n_status . " > " .  $o_crit;

  } elsif ( ( $o_warn != 0 ) && ( $o_warn > $DepCount) )  {
    	$n_status="WARNING";
...
sub check_options {
   ...
   if ( ! defined($o_warn) && ! defined($o_crit))
        { print "Put warning and / or  critical values!\n";
		usage();
		exit $ERRORS{"UNKNOWN"}
	}
    # Get rid of % sign
